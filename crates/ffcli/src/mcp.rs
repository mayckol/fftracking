...
fn dispatch(engine: &Engine, name: &str, a: &Value) -> ops::OpResult {
    ...
    match name {
        ...
        "pause" => ops::set_active(engine, &path()?, false),
        "resume" => ops::set_active(engine, &path()?, true),
...
fn tool_defs() -> Value {
    ...
    json!([
        ...
        tool("points", "List a folder's breaking points with added/modified/deleted counts.",
            ...
            &["path"]),
        tool("changes", "List files changed at a breaking point vs the previous point. Defaults to the latest point.",
        ...
        tool("diff", "Unified diff of a file at a breaking point. `now` compares the point to the current working tree.",
            json!({ "path": desc(&s("string"), path.1), "file": desc(&s("string"), "File path relative to the tracked folder"), "point": desc(&s("integer"), "Breaking-point id (default: latest)"), "now": desc(&s("boolean"), "Compare point vs working tree instead of previous point vs point") }),
            &["path", "file"]),
        ...
        tool("label", "Set a label on a breaking point.",
            ...
            &["path", "point", "text"]),
        tool("pause", "Pause tracking a folder (keeps history, stops capturing).",
            json!({ "path": desc(&s("string"), path.1) }), &["path"]),
        tool("resume", "Resume tracking a paused folder.",
            json!({ "path": desc(&s("string"), path.1) }), &["path"]),
